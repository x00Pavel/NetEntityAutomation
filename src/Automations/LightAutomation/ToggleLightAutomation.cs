...
{   
    ...
    {   
        _lightFsm = new ToggleLightFsm(config: config.FsmConfig, logger: logger)
        {
    ...
    protected sealed override void InitFsmTransitions()
    {
        ...
        MotionSensorEvent.Where(e => e.New?.State == "on").Subscribe(_ => _lightFsm.MotionOn());
        MotionSensorEvent.Where(e => e.New?.State == "off").Subscribe(_ => _lightFsm.MotionOff());
